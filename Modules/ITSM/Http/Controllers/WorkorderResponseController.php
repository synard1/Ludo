...
class WorkorderResponseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Decode existing data_details JSON to preserve all existing data
            $dataDetails = json_decode($workorder->data_details, true) ?: [];

            // Add duration to data_details while preserving existing data
            $dataDetails['duration'] = $duration;
            ...
            WorkOrder::where('id', $request->input('workorder_id'))
                ->update([
                    ...
                    'end_time' => $endTime,
                    'data_details' => json_encode($dataDetails),
                ]);
