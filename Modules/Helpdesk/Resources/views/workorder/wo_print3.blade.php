...
<table>
    ...
    <tr>
        ...
        <td>{{ $ticket->reporter_name ?? 'Nama Pemberi Tugas'}}</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        @if ($jumlah > 1)
            <td>
                @for ($x = 0; $x < $jumlah; $x++)
                {{ $x+1 .'. '. $ticket->issue_category[$x] ?? 'Nama Pemberi Tugas'}}<br>
                @endfor
            </td>
        @else
        <td>
            @for ($x = 0; $x < $jumlah; $x++)
            {{ $ticket->issue_category[$x] ?? 'Nama Pemberi Tugas'}}
            @endfor
        </td>
        @endif
    </tr>
    <tr>
        ...
        <td>{{ $ticket->subject ?? 'Nama Pemberi Tugas'}}</td>
    </tr>
</table>
...
<table>
    ...
    <tr>
        <th>End Time</th>
        <td>2023-01-01 11:30 AM</td>
    </tr>
    <tr>
        <th>Resolution</th>
        <td>Issue resolved by replacing the faulty power adapter.</td>
    </tr>
</table>
